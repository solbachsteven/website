...
(function() {
    ...
    // Skip in editor mode or nofx param
    if (window.location.search.indexOf('nofx') !== -1) return;
    var isEditor = window.location.href.match(/editor|edit|funnel\/manage/);
    if (isEditor) return;

    // === FX CSS ===
    var style = document.createElement('style');
    style.setAttribute('data-section', 'wm-fx');
    style.textContent = `
/* ======== FX EFFECTS ======== */
.wm-cursor-ring {
    position: fixed;
...
@media (max-width: 768px) {
    .wm-cursor-ring, .wm-glow { display: none !important; }
}
@media (min-width: 769px) {
    .wm-page { cursor: none; }
    .wm-page a, .wm-page button { cursor: none; }
}
`;
    ...
    ring.className = 'wm-cursor-ring';
    document.body.appendChild(ring);

    // FX Layers
    var glow = document.createElement('div');
    glow.className = 'wm-glow';
    document.body.appendChild(glow);

    var bgLayer = document.createElement('div');
    bgLayer.className = 'wm-fx-bg';
    document.body.appendChild(bgLayer);

    var fgLayer = document.createElement('div');
    fgLayer.className = 'wm-fx-fg';
    ...
    if (!isMobile) {
        ...
        document.addEventListener('mouseover', function(e) {
            if (e.target.closest('a, button, .wm-cta-primary'))
