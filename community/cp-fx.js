...
(function() {
    ...
    style.setAttribute('data-section', 'cp-fx');
    style.textContent = `
/* ======== FX EFFECTS ======== */
.cp-cursor-ring {
    position: fixed;
...
@media (max-width: 768px) {
    .cp-cursor-ring, .cp-glow { display: none !important; }
}
@media (min-width: 769px) {
    .cp-page { cursor: none; }
    .cp-page a, .cp-page button { cursor: none; }
}
`;
    ...
    bgLayer.className = 'cp-fx-bg';
    document.body.appendChild(bgLayer);

    var fgLayer = document.createElement('div');
    fgLayer.className = 'cp-fx-fg';
    ...
    if (!isMobile) {
        ...
        document.addEventListener('mouseover', function(e) {
            if (e.target.closest('a, button, .cp-cta-primary'))
